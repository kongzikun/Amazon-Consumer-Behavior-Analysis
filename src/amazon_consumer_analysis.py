...
# Resolve project directories relative to this file so the package works
...
class AmazonDataProcessor:
    ...
    def __init__(self, purchases_path: str | Path | None = None, survey_path: str | Path | None = None):
        """
    ...
    def load_data(self):
        ...
        if not self.purchases_path.exists():
            raise FileNotFoundError(
                ...
                "Place amazon-purchases.csv inside the data/ directory or pass a custom path."
            )

        if not self.survey_path.exists():
            raise FileNotFoundError(
                f"Survey data not found at {self.survey_path}. "
                "Place survey.csv inside the data/ directory or pass a custom path."
            )
    ...
    def preprocess_survey(self):
        ...
        demographic_cols = ['Q-demos-age', 'Q-demos-gender', 'Q-demos-income', 'Q-amazon-use-hh-size', 'Q-demos-education']
...
class DemographicAnalyzer:
    ...
    def create_demographic_dashboard(self):
        ...
        # Gender distribution
        gender_counts = self.df['Q-demos-gender'].value_counts()
        ...
        # Age distribution
        age_counts = self.df['Q-demos-age'].value_counts()
        fig.add_trace(go.Pie(
        ...
        # Income distribution
        income_counts = self.df['Q-demos-income'].value_counts()
        fig.add_trace(go.Pie(
        ...
        household_counts = self.df['Q-amazon-use-hh-size'].value_counts()
        fig.add_trace(go.Pie(
    ...
    def analyze_household_sharing(self):
        ...
        # 创建一个临时的数值型家庭规模列
        self.df['Household_Size_Numeric'] = self.df['Q-amazon-use-hh-size'].map(size_mapping)
        
        household_analysis = self.df.groupby(['Survey ResponseID', 'Q-amazon-use-hh-size', 'Household_Size_Numeric']).agg({
            'Order Date': 'count',
            'Total Amount': 'sum'
        }).reset_index()
        
        household_analysis.columns = ['User_ID', 'Household_Size', 'Household_Size_Numeric', 'Order_Count', 'Total_Spending']
        household_analysis['Orders_per_Person'] = household_analysis['Order_Count'] / household_analysis['Household_Size_Numeric']
        household_analysis['Spending_per_Person'] = household_analysis['Total_Spending'] / household_analysis['Household_Size_Numeric']
        
        # Create visualization
        fig = px.scatter(
            household_analysis,
            x='Household_Size_Numeric',
            y='Orders_per_Person',
            size='Total_Spending',
            color='Spending_per_Person',
            title='Account Sharing Analysis: Orders vs Household Size',
            labels={
                'Household_Size_Numeric': 'Household Size',
                'Orders_per_Person': 'Orders per Person',
                'Spending_per_Person': 'Spending per Person ($)'
            },
            hover_data=['Household_Size']  # 添加原始家庭规模信息到悬停提示
        )
...
class BehaviorAnalyzer:
    ...
    def cross_demographic_analysis(self):
        ...
        demographic_spending = self.df.groupby(['Q-demos-gender', 'Q-demos-age', 'Q-demos-income']).agg({
...
class EventAnalyzer:
    ...
    def covid_impact_analysis(self):
        ...
        # Add COVID start line as a shape instead of vline
        fig.add_shape(
            type="line",
            x0=self.covid_start,
            x1=self.covid_start,
...
class AdvancedSegmentation:
    ...
    def rfm_analysis(self):
        ...
        rfm_data = self.df.groupby('Survey ResponseID').agg({
            ...
            'Title': 'count'  # Frequency (using count of orders)
        }).reset_index()
...
def main() -> None:
    """CLI entry point for quick data validation."""
    ...
    processor.preprocess_purchases()
    processor.preprocess_survey()
    merged = processor.merge_datasets()

    validator = DataQualityValidator(merged)
    report = validator.generate_report()

    print("\nData quality summary:")
    print(f" - Total records: {report['total_records']:,}")
    if report['missing_columns']:
        print(" - Missing columns:", ", ".join(report['missing_columns']))
